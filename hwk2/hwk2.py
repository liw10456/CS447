...
Original file is located at
    https://colab.research.google.com/drive/1-ABmzGBkl92sGsGxLdPEKkU4Go23bOey
...
class CbowDataset(data.Dataset):
    ...
    def make_training_examples(self):
        ...
        for sentence in self.sentences:
            ...
            for i in range(self.context_size, len(words) - self.context_size):
                context = words[i - self.context_size:i] + words[i + 1:i + 1 + self.context_size]
                target_word = words[i]
    ...
    def get_context_vector(self, idx):
        ...
        context_indices = [self.word2idx[word] if word in self.word2idx else self.word2idx[CBOW_UNK] for word in context_words]
    ...
    def get_target_index(self, idx):
        ...
        return self.word2idx[target_word] if target_word in self.word2idx else self.word2idx[CBOW_UNK]
...
class CbowModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #     通過嵌入層，將每個詞的索引轉換為嵌入向量
        embeddings = self.embedding(inputs)  # embeddings size: [batch_size, 2*context_size, embed_size]
        ...
        #    平均上下文詞的嵌入向量
        context_embeddings = embeddings.mean(dim=1)  # size: [batch_size, embed_size]

        # 3. Feed this through your linear layer and then a ReLU activation. Expected size: [batch_size, hidden_size]
        #    通過第一個線性層和 ReLU 激活函數
        hidden = torch.relu(self.linear1(context_embeddings))  # size: [batch_size, hidden_size]
        ...
        #    通過輸出層
        output = self.output_layer(hidden)  # size: [batch_size, vocab_size]
...
class TextDataset(data.Dataset):
    ...
    def build_dictionary(self):
        ...
        for label, review in self.examples:
            for word in review:  # review 現在是一個單詞列表
                word_counts[word.lower()] += 1  # 轉換成小寫並計數
    ...
    def convert_text(self):
        ...
        for label, review in self.examples:
            review_ids = []
            for word in review:
                review_ids.append(self.word2idx.get(word, self.word2idx[CNN_UNK]))
            review_ids.append(self.word2idx[CNN_END])
            self.textual_ids.append(review_ids)
    ...
    def get_text(self, idx):
        ...
        review_ids = self.textual_ids[idx]
        if len(review_ids) > self.max_len:
            review_ids = review_ids[:self.max_len]
        else:
            review_ids += [self.word2idx[CNN_PAD]] * (self.max_len - len(review_ids))
        return torch.LongTensor(review_ids)
    ...
    def get_label(self, idx):
        ...
        return torch.LongTensor([1]) if label == 'pos' else torch.LongTensor([0])
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
        ...
        # Define multiple Convolution layers with different filter heights
        self.convs = nn.ModuleList([
            nn.Conv1d(in_channels=embed_size, out_channels=out_channels, kernel_size=height)
    ...
    def forward(self, texts):
        ...
        # Resulting shape: [batch_size, max_len, embed_size]
        embedded = self.embedding(texts)  # [batch_size, max_len, embed_size]

        # Transpose to have shape [batch_size, embed_size, max_len]
        embedded = embedded.permute(0, 2, 1)  # [batch_size, embed_size, max_len]

        # Pass through each convolution layer and compute their outputs
        conv_outputs = []
        for conv in self.convs:
            # Apply the convolutional layer
            conv_out = conv(embedded)  # [batch_size, out_channels, *]
            # Apply non-linearity (ReLU)
            conv_out = F.relu(conv_out)
            # Take the max value across the last dimension (to get the max pooling)
            pooled_out = F.max_pool1d(conv_out, conv_out.size(2)).squeeze(2)  # [batch_size, out_channels]
            conv_outputs.append(pooled_out)

        # Concatenate outputs from all CNNs
        cnn_output = torch.cat(conv_outputs, dim=1)  # [batch_size, out_channels * num_of_cnn_layers]

        # Apply dropout
        cnn_output = self.dropout(cnn_output)

        # Pass through the linear layer
        output = self.fc(cnn_output)  # [batch_size, num_classes]
