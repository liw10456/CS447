...
Original file is located at
    https://colab.research.google.com/drive/1PiyzHV33AreBtcLQtIjg4xW_FPLNu17I
...
class ParserConfiguration(object):
    def __init__(self, sentence, vocab):
        ...
        # Assert that the first word is the ROOT word (from vocab)
        assert sentence[0].word_id == self.vocab.WORD_ROOT
        self.stack = Stack([sentence[0]])  # Initialize stack with ROOT
        self.buffer = Buffer(sentence[1:])  # Initialize buffer with the rest of the sentence
    ...
    def parse_step(self, transition):
        ...
        '''
        # Ensure transition is valid according to vocab
        ...
        # Step 1: Handle the "S" transition (shift operation)
        if transition == 'S': # shift
          self.stack.push(self.buffer.pop())

        # Step 2: Handle the "LA-label" transition (left arc operation)
        ...
        # Step 3: Handle the "RA-label" transition (right arc operation)
        else: # right add arc
          pop_word = self.stack.pop()
          keep_word = self.stack.pop()
          self.stack.push(keep_word)
...
def get_gold_action(stack, buffer, gold_dependencies):
    action = None
    if len(stack) == 1:
        # 根據 buffer 是否有元素，直接返回相應的動作
        return 'S' if len(buffer) > 0 else 'DONE'
    ...
    if s2_arc.head == s1:
        return '-'.join(['LA', s2_arc.label])
    elif s1_arc.head == s2 and s1 not in bi_heads:
        return '-'.join(['RA', s1_arc.label])

    # 如果 buffer 還有元素，則返回 'S'
    if len(buffer) > 0:
        return 'S'

    # 默認返回 None（當沒有符合條件的情況時）
    return None
...
def get_top3_stack_features(parser_config):
    ...
    ### TODO ###
    for i in range(3):
        if i < len(parser_config.stack):  # Check if the stack has an element at index i
            word_features[i] = parser_config.stack.get_si(i + 1).word_id  # Get the word_id
            pos_features[i] = parser_config.stack.get_si(i + 1).pos_id   # Get the POS_id
...
def get_top3_buffer_features(parser_config):
    ...
    ### TODO ###
    for i in range(3):
        if i < len(parser_config.buffer):  # Check if the buffer has an element at index i
            word_features[i] = parser_config.buffer.get_bi(i + 1).word_id  # Get the word_id
            pos_features[i] = parser_config.buffer.get_bi(i + 1).pos_id   # Get the POS_id
...
def get_lc(parser_config, word):
    ...
    '''
    ### TODO ###
    # Filter left dependents
    left_dependents = [
        arc for arc in parser_config.dependencies
        if arc.head.idx == word.idx and arc.dependent.idx < word.idx
    ]

    # Sort the left dependents by the dependent word's index
    return sorted(left_dependents, key=lambda arc: arc.dependent.idx)
...
def get_rc(parser_config, word):
    ...
    '''
    ### TODO ###
    # Filter right dependents
    right_dependents = [
        arc for arc in parser_config.dependencies
        if arc.head.idx == word.idx and arc.dependent.idx > word.idx
    ]

    # Sort the right dependents by the dependent word's index in descending order
    return sorted(right_dependents, key=lambda arc: arc.dependent.idx, reverse=True)
...
def get_lc1_lc2_features(parser_config, i):
    ...
    ### TODO ###
    # Initialize feature lists with default NULL values
    vocab = parser_config.vocab

    # Attempt to retrieve the i-th stack element
    if len(parser_config.stack) >= i:
        si = parser_config.stack[-i]
        si_lc = get_lc(parser_config, si)

        # Assign features for the first and second left dependents if available
        for idx in range(min(2, len(si_lc))):
            word_features[idx] = si_lc[idx].dependent.word_id
            pos_features[idx] = si_lc[idx].dependent.pos_id
            label_features[idx] = si_lc[idx].label_id
...
def get_rc1_rc2_features(parser_config, i):
    ...
    if len(parser_config.stack) >= i:
        ...
        # Assign features for the first and second right dependents if available
        for idx in range(min(2, len(si_rc))):
            word_features[idx] = si_rc[idx].dependent.word_id
            pos_features[idx] = si_rc[idx].dependent.pos_id
            label_features[idx] = si_rc[idx].label_id
...
def get_llc_rrc_features(parser_config, i):
    ...
    def extract_features(dependencies, index):
        ''' Helper function to extract features for llc or rrc '''
        if dependencies:
            child_dependencies = get_lc(parser_config, dependencies[0].dependent) if index == 0 else get_rc(parser_config, dependencies[0].dependent)
            if child_dependencies:
                return (
                    child_dependencies[0].dependent.word_id,
                    child_dependencies[0].dependent.pos_id,
                    child_dependencies[0].label_id,
                )
        return vocab.WORD_NULL, vocab.POS_NULL, vocab.LABEL_NULL

    # Retrieve the i-th stack element
    if len(parser_config.stack) >= i:
        si = parser_config.stack[-i]

        # Get left-left child features
        si_lc = get_lc(parser_config, si)
        word_features[0], pos_features[0], label_features[0] = extract_features(si_lc, 0)

        # Get right-right child features
        si_rc = get_rc(parser_config, si)
        word_features[1], pos_features[1], label_features[1] = extract_features(si_rc, 1)
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embed_size, n_features, hidden_size, n_classes, dropout_prob):
        ...
        # Define a sequential block for the neural network layers
        self.fc_block = nn.Sequential(
            nn.Linear(n_features * embed_size, hidden_size),  # Map the embeddings to hidden space
    ...
    def forward(self, x):
        ...
        # Flatten the output from embedding layer and pass through the fully connected block
        x = x.view(x.size(0), -1)  # Flatten the input to shape [batch_size, n_features * embed_size]
        x = self.fc_block(x)  # Apply the sequential block
...
def select_best_legal_action(parser_configs, predictions, n_labels):
    ...
    ### TODO ###
    for p, parser_config in enumerate(parser_configs):
        # 初始化 predictions 為高概率，以便後續進行調整
        predictions[p, :] += 100

        # 檢查 parser_config 是否滿足特定條件來篩選合法的動作
        legal_actions = {
            'S': len(parser_config.buffer) == 0,  # illegal S action
            'RA': len(parser_config.stack) < 2,   # illegal RA action
            'LA': len(parser_config.stack) < 3    # illegal LA action
        }

        if legal_actions['S']:
            predictions[p, 2*n_labels] = 0  # Illegal S action
        if legal_actions['RA']:
            predictions[p, n_labels:2*n_labels] = 0  # Illegal RA action
        if legal_actions['LA']:
            predictions[p, :n_labels] = 0  # Illegal LA action
